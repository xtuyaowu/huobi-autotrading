...
from app.triangle_arbitrage.exchangeConnection.bitex import token_api
from app.triangle_arbitrage.exchangeConnection.bitex import key_api
import app.triangle_arbitrage.accountConfig

TOKEN_UPDATE_TIME = 600  # 提前此时间更新token


class BitexService(object):
    def __init__(self):
...
class BitexServiceAPIKey(object):
    ...
    def request_success(cls, ret):
        return BitexService.request_success(ret)
...
def init_bitex_account(key_index, api_key=True):
    if api_key:
        bitex_service = BitexServiceAPIKey(key_index=key_index)
        return bitex_service
    else:
        bitex_service = BitexService()
        acct_cfg = accountConfig.BITEX[key_index]
        ...
        if any([login_name, login_pwd, login_asset_pwd]) is None:
            return None
        bitex_service.set_login_name(login_name)
        bitex_service.set_pwd(login_pwd, True)
        bitex_service.set_asset_pwd(login_asset_pwd, True)
        return bitex_service